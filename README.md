...
10. **This is not an upgrade of the StartOS 0.3.x CryptPad package — it is a separate service.** That package is a different service underneath: six volumes under `/cryptpad/`, wrapping a CryptPad release from several years earlier, with nothing that maps across. StartOS carries it over the 0.4.0 update renamed to `cryptpad-legacy` and titled "CryptPad (Legacy)", so the two coexist rather than one being offered as the other's update. An operator exports their drive from the legacy instance, imports it here, and uninstalls the legacy one when satisfied. This package therefore has no migration to write and none to declare.
